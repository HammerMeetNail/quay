...
test('filtering is explicitly response-page scoped', async ({ page }) => { await page.goto(base); await page.getByLabel('Filter repositories on this page').fill('payments'); await expect(page.getByRole('link', { name: 'payments', exact: true })).toBeVisible(); await expect(page.getByText('1 shown · 25 loaded')).toBeVisible(); });
